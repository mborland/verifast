...
impl<T, A: Allocator> RawVec<T, A> {
    ...
    safety_proof {
        ...
        close <std::result::Result<(), collections::TryReserveError>>.own(_t, result);
    }
    @*/
    {
        //@ size_align::<T>();
...
const impl<A: [const] Allocator + [const] Destruct> RawVecInner<A> {
    ...
    ens thread_token(t) &*&
        match result {
            ...
            Result::Err(e) => <collections::TryReserveError>.own(t, e)
        };
    @*/
    /*@
    safety_proof {
        ...
        close <std::result::Result<RawVecInner<A>, collections::TryReserveError>>.own(_t, result);
    }
    @*/
    {
        ...
        let layout = match layout_array(capacity, elem_layout) {
            Ok(layout) => layout,
            Err(_) => {
                //@ leak <collections::TryReserveError>.own(_, _);
                //@ std::alloc::Allocator_to_own(alloc);
                //@ close <collections::TryReserveErrorKind>.own(currentThread, collections::TryReserveErrorKind::CapacityOverflow);
        ...
        let ptr = match result {
            ...
            Err(_) => {
                ...
                //@ close <collections::TryReserveErrorKind>.own(currentThread, err1);
...
safety_proof {
    ...
    close <std::result::Result<Layout, collections::TryReserveError>>.own(_t, result);
}
@*/
{
